...
void CheckCollision(std::pair<int, int> ball_coordinates,
                    std::pair<int, int> paddle_coordinates, Ball &ball,
                    Paddle &paddle) {
...
void PlayPong() {
  ...
  paddle.SetCoordinates(SCREEN_WIDTH - paddle.GetWidth() - 10,
                        SCREEN_HEIGHT / 2 - paddle.GetHeight() / 2);
